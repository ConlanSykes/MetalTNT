...
public class FrustumCuller {
  ...
  public boolean isRegionVisible(int regionX, int regionZ, int minY, int maxY) {
    ...
    if (!this.valid) {
      return true; // If frustum invalid, don't cull
    }

    // Convert region coords to block coords with a generous padding.
    // Chunk sections are 16x16x16 but biome blending and vertex positions
    // can extend slightly outside. Adding 16 blocks of padding on each
    // side prevents over-culling at frustum edges.
    float minX = (float) regionX * 16.0F - 16.0F;
    float minZ = (float) regionZ * 16.0F - 16.0F;
    float maxX = minX + 48.0F; // 16 padding + 16 section + 16 padding
    float maxZ = minZ + 48.0F;
    float minYf = (float) minY - 16.0F;
    float maxYf = (float) maxY + 16.0F;
